...
| 모듈                  | 진행 상황                                                                |
| --------------------- | ------------------------------------------------------------------------ |
| **AuthServerManager** | ✅ 개발 완료 (외부오픈 및 월드 상태 Redis 관리 )                         |
...
> 내부 서버 통신(Auth<->World, Auth<->AuthDB, World <-> WorldDB)은 Kafaka로 변경
> AuthServer 계정 제제 확인 프로세스 추가
> WorldServer 브로드 캐스팅 기능 추가
> WorldServer AOI 설정 방법 설계 및 기능 추가
